    ...
    for batch_idx, (input_ids, labels) in enumerate(train_dataloader):
        ...
        try:
            ...
            if "out of memory" in str(exception):
                ...
                #     torch_directml.torch.clear_autocast_cache()
            else:
...
def main():
    ...
    # 当用户使用GPU,并且GPU可用时
    args.cuda = torch_directml.is_available() and not args.no_cuda
    device = torch_directml.device() if args.cuda else 'cpu'
    args.device = device
    ...
    # 并行训练模型
    if args.cuda and torch_directml.device_count() > 1:
